...
#### 优雅重启（[https://segmentfault.com/a/1190000038461407](https://segmentfault.com/a/1190000038461407)）

* go 1.8.x后，golang在http里加入了shutdown方法，用来控制优雅退出。
* 原进程（父进程）先`fork`一个子进程，同时让`fork`出来的子进程继承父进程所监听的`socket`。
* 子进程完成初始化后，开始接收`socket`的请求。
* 父进程停止接收新的请求，并将当下的请求处理完，等待连接空闲后，平滑退出。
